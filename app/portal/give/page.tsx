...
export default function GivePage() {
  ...
  return (
    <div>
      ...
      {/* Trust strip */}
      <div className="surface p-4 flex items-center gap-3 mb-6">
        <ShieldIcon
          width={20}
          height={20}
          className="text-[color:var(--burgundy-2)] shrink-0"
        />
        <p className="text-xs text-white/60 leading-relaxed">
          Payments are processed securely by Paystack. Your details are never
              ...
              <div className="flex items-center justify-between">
                ...
                {selected && (
                  <CheckIcon
                    width={16}
                    height={16}
                    className="text-[color:var(--burgundy-2)]"
                  />
                )}
              </div>
              <p className="text-[11px] text-[color:var(--burgundy-2)] mt-1">
                {c.scripture}
              </p>
      ...
      <div className="grid grid-cols-3 gap-2 mb-3">
        {AMOUNT_PRESETS.map((a) => (
          <button
            key={a}
            onClick={() => setAmount(a)}
            className={`h-12 rounded-xl text-sm font-medium tabular transition-colors border ${
              amount === a
                ? "btn-primary border-transparent"
                : "border-[color:var(--hair-strong)] text-white/80 hover:bg-white/5"
            }`}
          >
            {naira(a)}
          </button>
      ...
      <button
        ...
        className="btn-primary w-full h-14 rounded-full text-sm font-medium flex items-center justify-center gap-2 disabled:opacity-50"
      >
      ...
      <h2 className="eyebrow mt-8 mb-3">Prefer a Bank Transfer?</h2>
      <div className="surface p-5 space-y-3">
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            ...
            <div className="text-base font-medium tabular tracking-wide">
        ...
        <p className="text-xs text-white/40 pt-1 hair-t">
      ...
      <button
        ...
        className="card-tap w-full flex items-center gap-4 p-4 mt-6"
      >
        ...
        <span className="flex-1 text-left">
          ...
          <span className="block text-xs text-white/50 mt-0.5">
            {store.gifts.length} record{store.gifts.length === 1 ? "" : "s"} •{" "}
            {naira(total)} total
          </span>
        </span>
        ...
        {confirm && (
          <div className="space-y-5">
            <div className="surface p-5 text-center">
              <div className="eyebrow mb-1">{confirm.category}</div>
      ...
      >
        <div className="space-y-4">
          <div className="surface p-5 flex items-center justify-between">
            <div>
              <div className="eyebrow">Total given</div>
