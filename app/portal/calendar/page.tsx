  ...
  if (!now) {
    return (
      <div className="min-h-[60vh] grid place-items-center">
        <span className="font-display text-xl text-white/30 animate-pulse">KW</span>
      </div>
    );
  }
          ...
          {cells.map((d, i) => {
            ...
            return (
              ...
              >
                {d}
                {hasEvent && !isToday && (
                  <span className="absolute bottom-1 w-1 h-1 rounded-full bg-[color:var(--burgundy-2)]" />
                )}
              </div>
            );
          })}
          ...
          {orderedEvents.map((ev) => (
            <div key={ev.id} className="flex items-center gap-4 p-4">
              <div className="shrink-0 w-16">
                <div className="text-sm font-medium text-[color:var(--burgundy-2)]">
